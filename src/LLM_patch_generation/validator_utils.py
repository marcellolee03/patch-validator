...
def generate_validator_prompt(env_info, vuln_details, generated_patches):
    return f"""
# Role:
...
# Task:
Analyze three (3) proposed patches designed to fix a specific security vulnerability. 
Your goal is to evaluate each patch against the provided context, assign a score, and determine the winner(s).
...
* **Score 2 (Sub-optimal Fix):** The patch successfully fixes the vulnerability but uses a "workaround" logic, introduces technical debt, or has minor side effects.
* **Score 3 (Optimal Fix):** The patch effectively fixes the vulnerability, is minimalistic/surgical, and maintains code quality/performance standards.
